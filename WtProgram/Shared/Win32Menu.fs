...
module Win32Menu =
    ...
    let show (hwnd:IntPtr) (pt:Pt) (scale:float) (items:List2<_>) (enableDarkMode:bool) =
        ...
        let closeOnOutsideInput (cursor: POINT) =
            ...
            if WinUserApi.GetForegroundWindow() <> foregroundAtOpen || (pressed && not (overMenu ())) then
                EndMenu() |> ignore
        ...
        let mouseHook = WinUserApi.SetWindowsHookEx(WindowHookTypes.WH_MOUSE_LL, mouseHookProc, WinBaseApi.GetModuleHandle(IntPtr.Zero), 0)
