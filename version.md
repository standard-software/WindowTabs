...
- A group comes back even when its applications start long after WindowTabs does, which is the normal case for anything not in Startup: the windows that are not open yet are held aside, and each one joins its group the moment it appears, however much later that is
- A group whose windows are ALL still closed is held together too - the first one to appear forms the group and the rest join it in their saved order, instead of each landing in a group of its own
- Windows still waiting to open are kept in the settings file until they do, so a WindowTabs restart or a second Windows restart in between no longer loses the group. One that has not reopened in thirty days gives its place up and the entry goes. A tab closed by hand is kept the same way for eight days, so closing a tab and opening the window again the following week still puts it back in its group at its old place, with its name and colours - where before that only held until WindowTabs was next restarted. Emptying a group still ends it: once its last tab is closed nothing is kept
- A window whose title did not survive the restart is found by its application and the position it reopened at. A browser shows its active tab in the title, so Edge or Chrome could come back reading differently and be left out of a group the others had already rejoined. Where several windows of one application cannot be told apart that way, their entries are held until the titles have settled, so each window goes back to its own place instead of a neighbour's. A window that was minimized when the state was saved is matched by its title alone, its recorded position being the off-screen corner Windows parks minimized windows in. Each window takes one saved entry at most, so one already back in its place cannot go on to take the entry of a sibling that never reopened and move itself into that sibling's slot
...
- Rename, pin, colors and alignment come back with their window across a Windows restart, where before they were dropped for every window whose application had not started yet. They are applied when the exe path and title pick out exactly one saved window; where a window could only be matched by its application and more than one entry could have been meant, they are left off rather than attached to the wrong one. The alignment a window is restored to is written back to the saved state as well, so the next restart no longer brings back the alignment the window had briefly inherited from its neighbour while it waited to be recognised
...
- Tab context menu: renamed "Tab Position" to "Tab Alignment" in all languages so it no longer overlaps with "Position Move"
- Tab context menu: "Rename this tab", "Reset this tab name" and "Copy window title" show their name suffix again (their values can differ from the target-tab header); the reset item hides the name while disabled
- Multi-select labels: when the right-clicked tab is not part of the selection, items now read "Selected {N} tabs + target tab ..." instead of silently counting it into {N}
