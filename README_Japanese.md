...
スマートフォンやタブレット端末でブラウザ閲覧やエンターテイメントを楽しめる時代に、わざわざ Windows PC を使うのは、きっと、そのような創造的なことを取り組まれているのだと思います。

そういう中で Windows の操作性を向上させたいということに興味があるのでしたら、WindowTabs をよい道具として使っていただけると思います。そのようなニーズに応えられるような機能を作りこんでバージョンを重ねてきました。

---

私は WindowTabs がオープンソースになる前から WindowTabs の有料ユーザーでタブUIをとても気に入っていました。
...
Microsoft は、以前に Windows の OS の機能拡張の試作として Sets と名付けた機能でウィンドウの全てをタブ管理することを目指していたことがありました。ですが、中止になったようです。理由は不明ですが、OS の内部に組み込むには過去の互換性などで難しすぎたのでしょう。ですが、目指す操作性は素晴らしいです。WindowTabs は Sets の前から、OS の内部には組み込まれない形で、タブUIを実現しています。
