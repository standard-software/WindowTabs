...
- マルチディスプレイ時は、位置移動系メニュー (「位置移動」「新しいタブの位置指定」「タブ分離の位置指定」) がディスプレイごとに1つずつ表示されます (例:「位置移動 左ディスプレイ」)。ウィンドウが現在あるディスプレイには「 \*」が付きます。他のディスプレイのメニューは先頭に「このディスプレイと同じ位置」項目が付きます。
...
- 指定したタブを分離 (「このタブの分離 : (タブ名)」) して他の位置に移動したり、他のタブグループに連結することができます。
