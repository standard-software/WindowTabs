...
- 新しいタブ : (exe名) 起動 — このタブの右、位置指定、他のグループへ連結
  - 位置指定は「位置移動」と同じサブメニューで、先頭に「同じ位置」項目あり
- 位置移動
...
- このタブの分離 — 位置指定 (「位置移動」と同じサブメニュー)、他のグループへ連結
- タブを閉じる — このタブ、左側、右側、他のタブ、全てのタブ
- スナップ時のタブの余白 — 上に余白をあける
- タブの配置 — このタブまたは全てのタブを左寄せ・右寄せ
- タブのピン止め — このタブまたは全てのタブをピン止め・解除
- タブの色設定 — 赤 / 青 / 緑 / 黄色 / 紫 / オレンジ / ピンクの背景・下線・枠線。このタブまたは全てのタブの色を解除可能
- タブ名の編集 — 名前を変更・リセット
- システム
  - (exe名) のパスをコピー
  - ウィンドウタイトルをコピー : (タイトル)
...
個別タブ向け項目のタブ名表示は廃止し、メニュー先頭の「対象タブ」で対象を示すようにしました。言語ファイルで `{TabName}` を使えば従来の「... : (タブ名)」表示に戻せます 
