...
- タブストリップと設定ダイアログはモニタごとの DPI に対応: 拡大率を上げたディスプレイでもネイティブ解像度で描画され (ビットマップ拡大によるぼやけ無し)、表示中のモニタの拡大率に追従
...
>
> **ss_2026.08.21 で鮮明化:** 従来、拡大率を上げたディスプレイではタブストリップと設定ダイアログが Windows のビットマップ拡大でぼやけていましたが、各モニタのネイティブ解像度で描画するようになりました。
...
- 再起動なしで言語を変更可能
- トレイメニューから言語を変更
- `WindowTabs.exe` と同じ場所の `Settings\Language\` にある言語ファイルは既定値で、バージョンアップのたびに上書きされます。自分のファイルは `%APPDATA%\WindowTabs\Settings\Language\` に置きます。ここはバージョンアップで触られません
  - 言語を追加するには、`MyLanguage.json` と、それを載せた `FileList.json` をここに置きます
  - 同梱の訳語を直すには、同じ名前のファイルに直したいキーだけを書きます。他の文字列は同梱のまま使われ、新しいバージョンで追加された文字列もそのまま届きます
  - ここに置いた `FileList.json` は同梱の一覧を丸ごと置き換えるので、トレイメニューに出す言語を絞ることもできます
  - フォルダを消せば同梱の状態に戻ります
- `Settings\` の他のファイル（`WindowMargin.json`、`VersionFolder.json`）も同じです。`%APPDATA%\WindowTabs\Settings\` に同じ名前のファイルを置くと、項目単位で同梱のものに重なります
...
- 状態は10秒ごとに保存されるため、強制終了（タスクマネージャーでの終了、クラッシュ、電源断）でも失われません。
